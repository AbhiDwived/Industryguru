...
export default function VendorSubSlug() {
  const dispatch = useDispatch();
  const allSlugs = useSelector((state) => state.VendorSlugStateData);
  const allSubSlugs = useSelector((state) => state.VendorSubSlugStateData);
  ...
  useEffect(() => {
    if (selectedSlug) {
      dispatch(getVendorSubSlugByParent({ parentSlugId: selectedSlug }));
    } else {
  ...
  const handleCreateSubSlug = async () => {
    ...
    dispatch(addVendorSubSlug(payload));
    setNewSubSlug("");
  };
  ...
  const handleAddInnerSubSlug = async () => {
    ...
    if (subSlugToUpdate) {
      ...
      dispatch(updateVendorSubSlug({ ...subSlugToUpdate, innerSubSlugs: updatedInnerSubSlugs }));
      setNewInnerSubSlug("");
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this sub-slug?")) {
      dispatch(deleteVendorSubSlug({ _id: id }));
    }
  };
  ...
  const handleUpdateSubSlug = () => {
    ...
    dispatch(updateVendorSubSlug(updatedSubSlugData));
