...
import { addVendorSlug, deleteVendorSlug, getVendorSlug, updateVendorSlug } from "../../Store/ActionCreators/VendorSlugActionCreators";

export default function VendorSlug() {
  const dispatch = useDispatch();
  const allSlugs = useSelector((state) => state.VendorSlugStateData);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    dispatch(addVendorSlug({ name: formData.slug, slug: formData.slug }));
    setFormData({ slug: "" });
  };
  ...
  const handleAddInnerSlug = async () => {
    ...
    if (slugToUpdate) {
      ...
      dispatch(updateVendorSlug({ _id: slugToUpdate._id, name: slugToUpdate.name, slug: slugToUpdate.slug, innerSlugs: updatedInnerSlugs }));
      setNewInnerSlug("");
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this slug?")) {
      dispatch(deleteVendorSlug({ _id: id }));
    }
  };
  ...
  const handleUpdateSlug = () => {
    ...
    dispatch(updateVendorSlug(updatedSlugData));
